...
coordinate ObstacleDistanceGrid::cellIndexLookup(const pose_xyt_t& p) const{
    int x = (int)floor((p.x - globalOrigin_.x) / metersPerCell_);
    int y = (int)floor((p.y - globalOrigin_.y) / metersPerCell_);
    coordinate result;
    result.x = x;
    result.y = y;
    return result;
